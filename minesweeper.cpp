...
void set_mines(string **board, int rows, int cols, int mines)
{
	...
	while (count != mines)
	{
		...
		int y = rand() % rows;
		if (board[y][x] != "*")
		{ //make sure mine isnt already there
			board[y][x] = "*";
			count++;
		}
	}
...
{
	if ((i < rows - 1) && (j > 0))
	{																	//dont check if on top or left edge of board
		if (board[i + 1][j - 1] == "*") //top left
			num++;
	}
	if ((j > 0) && (i > 0))
	{																	//dont check if on bottom or left edge of board
		if (board[i - 1][j - 1] == "*") //bottom left
			num++;
	}
	if ((i < rows - 1) && (j < cols - 1))
	{																	//dont check if on top or right edge of board
		if (board[i + 1][j + 1] == "*") //top right
			num++;
	}

	if ((i > 0) && (j < cols - 1))
	{																	//dont check if on bottom or right edge of board
